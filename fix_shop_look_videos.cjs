...
const regex = /const videos = \[\s*'[^]*?'\s*\];/m;

const replacement = `const productVideos: Record<string, string> = {
...
const regex2 = /<video[\s\S]*?className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"\s*\/>/;
...
code = code.replace(regex2, newVideo);
